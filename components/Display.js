	...
	for (let i = 0; i < monthDays / 7; i++) {
		...
		for (let j = 0; j < 7; j++) {
			...
			month[i][j].actions = actions.filter(
				(action) =>
					action.date.format("D/M/YYYY") ===
					currentDay.format("D/M/YYYY")
			);
				...
				<div>
					<button
						...
						onClick={() => {
							if (
								confirm(
									"Deseja realmente excluir essas " +
										selected.length +
										" ações? Essa ação não pode ser desfeita."
								)
							) {
								selected.map((item) => {
									deleteAction(
					...
					>
						Excluir essas {selected.length} ações
					</button>
				</div>
			) : null}
...
const Filters = ({
	...
	accounts = [
		{ id: 1000, name: "Todas as Contas", slug: "all" },
		...accounts,
	];
			...
			<div className="grid grid-cols-7 border-t border-b">
				{["DOM", "SEG", "TER", "QUA", "QUI", "SEX", "SÁB"].map(
					(day) => (
						<div
							key={day}
							className="p-4 text-sm font-bold tracking-wider"
						>
							{day}
						</div>
					)
				...
				<tbody>
					{actions.map((action) =>
						...
						(account.slug === "all" ||
							action.account.slug === account.slug) ? (
							<tr
								className="text-sm border-t group"
								key={action.id}
							>
								...
								<td>
									<div className="flex items-center justify-end pr-8">
										<span className="flex items-center -space-x-2">
											{action.profiles_responsible.map(
												(responsible) => (
													<Avatar
														key={responsible.id}
														avatar={responsible}
...
const Grid = ({
	...
	actions = actions.filter(
		(action) =>
			action.tags.filter(
				(tag) => tag.slug === "post" || tag.slug === "reels"
			).length > 0
	);
	console.log(actions);
	actions = actions.filter(
